...
The `ROUTE_TOOL_DEFINITION` SHALL be generated dynamically from the loaded department configuration. The `department` parameter enum SHALL be built from the keys of the `departments` dict in the config. The tool description SHALL be generated dynamically from each department's description and triggers.
...
The system SHALL validate department names from function call arguments against the set of department keys loaded from config. The static `Department(str, Enum)` is replaced by a `valid_departments: set[str]` attribute on the config. `parse_function_call_action` SHALL accept the valid set and reject unknown department names.
