...
- **Restructure `router_prompt.yaml`** into 5 clearly separated layers:
  ...
  - `departments`: Structured department definitions with `description`, `triggers`, and `tool` (specialist function link). Single source of truth for routing — replaces the current duplicated `decision_rules` + `departments` sections.
...
- `coordinator`: Specialist tool name resolution changes from hardcoded `f"specialist_{department}"` to reading `tool` field from department config.
...
- **`backend/src/voice_runtime/coordinator.py`** — specialist tool name resolution (~line 687)
- **Tests** — `test_model_router.py`, `test_two_step_routing.py` need updating for new YAML structure
