...
The bridge SHALL translate Coordinator output events into OpenAI Realtime API WebSocket messages.

#### Scenario: Prompt with message array sent as session.update + response.create
- **WHEN** `send_voice_start()` is called with a prompt containing a message array (system + user messages)
- **THEN** the bridge SHALL send a `session.update` with `instructions` set to the combined system messages, then send a `response.create` with the user message as input
...
- **THEN** the bridge SHALL send `{"type": "response.cancel"}` over the WebSocket
