...
**Reason**: Browser connects directly to OpenAI via WebRTC; ICE candidates are exchanged between browser and OpenAI, not via the backend.
**Migration**: Remove `POST /calls/{call_id}/ice` endpoint and all client-side ICE candidate posting.

### Requirement: STUN/TURN configuration
**Reason**: The backend no longer manages WebRTC peer connections. STUN/TURN is handled by the browser and OpenAI's ICE infrastructure.
**Migration**: Remove `STUN_SERVERS`, `TURN_SERVERS`, `TURN_USERNAME`, `TURN_CREDENTIAL` from settings.
