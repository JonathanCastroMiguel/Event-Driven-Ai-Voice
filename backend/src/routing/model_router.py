...
The router prompt is sent with `tool_choice="required"`, which forces the
model to emit only a `route_to_specialist` function call and suppresses
audio output for that response. The spoken reply is generated by a separate
follow-up `response.create` (direct path, handled by the bridge) or by a
specialist text model + a "say exactly" follow-up (specialist path, handled
by the coordinator).
"""
...
SYSTEM_MECHANIC: str = (
    "Classify every user message by calling route_to_specialist.\n"
    "You MUST call this function for every message — no exceptions.\n\n"
    "Use department=\"direct\" for messages you can answer yourself "
    "(greetings, smalltalk, simple questions, guardrails, out-of-scope) "
    "and a specialist department (sales/billing/support/retention) for "
    "anything that requires specialist knowledge.\n\n"
    "The function call itself is silent. The spoken reply is generated by a "
    "separate follow-up response, so do not attempt to speak in this response."
)
...
class RouterPromptBuilder:
    ...
    def get_department_filler(self, department: str) -> str | None:
        ...
        if agent_config is None or not agent_config.fillers:
            return None
        return random.choice(agent_config.fillers)  # noqa: S311  # filler selection is not security-sensitive
