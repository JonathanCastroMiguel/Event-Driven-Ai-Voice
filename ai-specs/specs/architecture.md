...
| `ModelRouterAction` | `call_id, department, summary, ts` | Model called `route_to_specialist()` with a specialist department (not `direct`); the spoken reply is generated by a follow-up response |
...
2. **Specialist prompt** (text model → directive vocalization): Specialist tools call a text model (`gpt-4o`, configurable via `specialist_model`) with the department system prompt + triage framework + conversation history. The text model returns a plain text response (1-2 sentences). The Coordinator wraps this text in a `response.create` dict with a directive instruction ("Say exactly the following to the customer...") so the Realtime model vocalizes it literally without paraphrasing. If the text model call fails (timeout, HTTP error, empty response), the tool falls back to returning a `response.create` dict with raw instructions for the Realtime model to interpret (legacy behavior). Each department has its own system prompt with department-specific triage examples.
...
**Specialist Tools** (`backend/src/voice_runtime/specialist_tools.py`): Provides text-model-based triage for specialist departments. A shared `httpx.AsyncClient` is configured at startup via `configure(api_key, model, timeout_s)` and closed at shutdown via `close()`. The core function `_call_text_model(system_prompt, user_message) -> str | None` POSTs to `/v1/chat/completions` with `max_tokens=200`, `temperature=0.8`. Four department system prompts (`_SALES_SYSTEM_PROMPT`, `_BILLING_SYSTEM_PROMPT`, `_SUPPORT_SYSTEM_PROMPT`, `_RETENTION_SYSTEM_PROMPT`) contain department-specific triage examples. A shared `_TRIAGE_FRAMEWORK` constant enforces: (1) mandatory clarifying question before transfer, (2) step-awareness from conversation history, (3) language matching. The `_run_specialist(department, summary, history)` function calls `_call_text_model` with the department prompt; on success returns `str`, on failure falls back to a `response.create` dict via `_wrap_response_create`. Public functions (`specialist_billing`, etc.) delegate to `_run_specialist`. Config: `specialist_model` (default `gpt-4o`), `specialist_timeout_s` (default 5.0), reuses `openai_api_key`.
...
**Filler strategy**: Selects a contextual filler per department via `RouterPromptBuilder.get_department_filler(department)` (picks randomly from the department's `fillers` list in the JSON config). Departments with empty fillers (e.g., `direct`) skip filler emission. The filler is emitted as a `RealtimeVoiceStart` with a `str` prompt wrapped in a directive instruction that tells the Realtime model to say it in the customer's language. Fillers play in parallel with the specialist text-model call (~500-1500ms), making them genuinely useful for masking latency. A 1200ms auto-cancel timeout ensures the filler is cut when the specialist response arrives.
...
**Function call routing**: The bridge handles `response.function_call_arguments.done` events from the OpenAI Realtime API. When the model calls `route_to_specialist()`, the bridge calls `parse_function_call_action()` to validate and parse the function call arguments. The classification response carries no audio (the router prompt sets `tool_choice="required"`, which suppresses speech in that response), so the bridge handles two distinct follow-up paths:

- `department="direct"` → the bridge sets `_pending_direct_audio = True`, captures `_pending_fn_call_id` / `_pending_fn_item_id`, and on `response.done` acknowledges the function call (via `function_call_output`) and sends a second `response.create` **without tools** to obtain the spoken reply. No `model_router_action` is emitted; the Coordinator only sees the final `voice_generation_completed`.
- specialist department (e.g. `billing`) → the bridge sets `_function_call_received = True` and emits a `model_router_action` EventEnvelope with `department` and `summary`. The Coordinator drives the specialist flow (config filler + text-model call + "say exactly" follow-up).

**Transcript cleanup**: The model occasionally vocalizes function call syntax in its audio output (e.g., `(functions.route_to_specialist(department="billing", ...))`). The `_clean_transcript()` helper strips leaked function call text from transcripts using a regex pattern. Applied to the accumulated `_response_transcript_buffer` before it is emitted in `voice_generation_completed`. This is a post-hoc text cleanup only — the audio itself cannot be modified after generation.
...
| `response.audio_transcript.delta` | *(no envelope)* — accumulated in `_response_transcript_buffer`; emitted as `transcript` in `voice_generation_completed`. Empty for the classification response (`tool_choice="required"` suppresses audio); populated for direct follow-up and specialist responses. |
...
| `_response_transcript_buffer` | Accumulates `response.audio_transcript.delta` text for the agent's spoken response (reset on each `response.created`); included as `transcript` in `voice_generation_completed`. |
...
**Session lifecycle:**
1. `POST /calls` creates a per-call runtime actor stack: Coordinator, TurnManager, AgentFSM, ToolExecutor, and RealtimeEventBridge. The shared `RouterPromptBuilder` singleton is injected into the Coordinator. Specialist tools are configured at application startup via `specialist_tools.configure()` (shared httpx client for text model calls). The bridge is wired to the Coordinator bidirectionally:
...
6. Bridge receives response.function_call_arguments.done event
   ...
   → Emits model_router_action event with department and summary
